...
export default function AdminDashboard() {
		...
		isClient && viewGallery
			? createPortal(
					<div className="fixed inset-0 z-[1500] flex items-center justify-center bg-slate-950/70 p-6 backdrop-blur">
						<div className="flex h-full max-h-[90vh] w-full max-w-6xl flex-col overflow-hidden rounded-3xl border border-slate-800 bg-slate-900 shadow-2xl">
							<header className="flex flex-col gap-3 border-b border-slate-800 px-6 py-5 md:flex-row md:items-center md:justify-between md:px-8">
								<div>
									<p className="text-xs uppercase tracking-[0.25em] text-slate-400">Gallery</p>
								...
								<div className="flex items-center gap-3">
									<span className="rounded-full bg-slate-800 px-4 py-2 text-xs font-semibold uppercase tracking-wide text-slate-200">
										{viewGallery.photoCount} photos
									</span>
									<button
										type="button"
										onClick={() => setViewGallery(null)}
										className="inline-flex h-11 w-11 items-center justify-center rounded-full bg-slate-800 text-2xl font-semibold text-slate-200 transition hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-main/40"
									>
							...
							<div className="flex-1 overflow-y-auto px-6 pb-6 pt-4 md:px-8">
								{viewGallery.photos.length === 0 ? (
									<div className="flex h-full flex-col items-center justify-center gap-3 rounded-2xl border border-dashed border-slate-700 bg-slate-900/50 p-10 text-center text-slate-300">
									...
									<div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
										{viewGallery.photos.map((photo) => {
											...
											return (
												<div
													key={photo.id}
													className="group overflow-hidden rounded-2xl border border-slate-800 bg-slate-900 shadow-md transition hover:shadow-lg"
												>
													<div
													...
													<div className="flex flex-col gap-2 px-4 py-3 text-sm font-semibold text-slate-200">
														<div className="flex items-center justify-between gap-2">
															<span className="truncate">{photo.image_code}</span>
															{isSelected ? (
																<span className="text-xs text-main text-main/80">Client pick</span>
															) : null}
														</div>
														{hasDimensions ? (
								...
								<div className="mt-6 rounded-xl border border-slate-800 bg-slate-950/40 p-4 text-sm text-slate-300">
									<p className="font-semibold text-slate-200">Selected codes</p>
									{viewGallery.selectedCodes.length === 0 ? (
										<p className="mt-1 text-xs text-slate-400">No selections yet.</p>
									) : (
										<div className="mt-2 flex flex-wrap gap-2">
											{viewGallery.selectedCodes.map((code) => (
												<span
													key={code}
													className="inline-flex items-center rounded-full bg-main/20 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-main text-main/90"
												>
													{code}
												</span>
											))}
	...
	if (isCheckingSession) {
		return (
			<section className="mx-auto flex w-full max-w-md flex-col items-center justify-center gap-4 rounded-2xl bg-slate-900/80 p-12 text-center shadow-xl">
				<div className="h-12 w-12 animate-spin rounded-full border-4 border-main border-t-transparent" />
				<p className="text-sm font-semibold text-slate-200">Loading dashboard...</p>
			</section>
		)
	}
	...
	if (!session) {
		return (
			<section className="mx-auto w-full max-w-md rounded-2xl border border-slate-800 bg-slate-900/80 p-10 shadow-xl backdrop-blur">
				...
				<form className="mt-8 space-y-5" onSubmit={handleLoginSubmit}>
					<div className="space-y-2">
						<label className="text-sm font-semibold text-slate-200" htmlFor="username">
							Username
						</label>
						<input
							id="username"
	...
	return (
		<>
			<section className="mx-auto flex w-full max-w-7xl flex-col gap-8 rounded-3xl border border-slate-800 bg-slate-900/80 p-8 shadow-2xl backdrop-blur md:p-14">
				...
				<section className="grid gap-6 lg:grid-cols-2">
					<div className="rounded-2xl border border-slate-800 bg-slate-900/80 p-6 shadow-md">
						<h2 className="text-lg font-semibold text-slate-100">Create a new gallery</h2>
						...
						<form className="mt-6 space-y-4" onSubmit={handleCreateGallery}>
							<div>
								...
								<input
									...
									className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-800 px-4 py-3 text-sm font-medium text-slate-50 outline-none transition focus:border-main focus:ring-2 focus:ring-main/40"
							...
							{createGalleryMessage ? (
								<p className="text-sm text-main text-main/80">{createGalleryMessage}</p>
							) : null}
							{createGalleryPin ? (
								<div className="rounded-xl border border-dashed border-main/60 bg-main/20 p-4 text-sm font-semibold text-main text-main/90">
					...
					<div className="rounded-2xl border border-slate-800 bg-slate-900/80 p-6 shadow-md">
						<h2 className="text-lg font-semibold text-slate-100">Upload photos</h2>
						<p className="mt-1 text-sm text-slate-300">
						...
						<form className="mt-6 space-y-4" onSubmit={handleUploadPhoto}>
							<div>
								<label className="text-sm font-semibold text-slate-200">Select gallery</label>
								<select
									className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-800 px-4 py-3 text-sm font-medium text-slate-50 outline-none transition focus:border-main focus:ring-2 focus:ring-main/40"
							...
							<div>
								<label className="text-sm font-semibold text-slate-200">Select files</label>
								<input
									ref={uploadInputRef}
									type="file"
									multiple
									...
									className="mt-1 w-full rounded-lg border border-dashed border-slate-700 bg-slate-800 px-4 py-3 text-sm text-slate-200 outline-none transition focus:border-main focus:ring-2 focus:ring-main/40"
								/>
							...
							{uploadMessage ? (
								<p className="text-sm text-main text-main/80">{uploadMessage}</p>
							) : null}
				...
				<section className="rounded-2xl border border-slate-800 bg-slate-900/80 p-6 shadow-md">
					<div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
						<div>
							<h2 className="text-lg font-semibold text-slate-100">Galleries</h2>
							<p className="text-sm text-slate-300">
								Review galleries, uploaded photos, and client selections.
							</p>
						</div>
						{isLoadingGalleries ? (
							<span className="text-xs uppercase tracking-wide text-slate-400">Refreshing...</span>
						) : (
							<button
								type="button"
								onClick={loadGalleries}
								className="inline-flex items-center gap-2 rounded-full border border-slate-700 px-3 py-2 text-xs font-semibold uppercase tracking-wide text-slate-200 transition hover:border-main hover:text-main focus:outline-none focus:ring-2 focus:ring-main/40"
							>
								Refresh
							</button>
						)}
					...
					<div className="mt-6 space-y-6">
						{galleries.length === 0 ? (
							...
							galleries.map((gallery) => (
								<article
									key={gallery.id}
									className="rounded-2xl border border-slate-800 bg-slate-900/80 p-6 shadow"
								>
									<header className="flex flex-col gap-1 md:flex-row md:items-center md:justify-between">
										...
										<div className="rounded-full bg-slate-800 px-4 py-2 text-xs font-semibold uppercase tracking-wide text-slate-200">
											{gallery.photoCount} photos
										</div>
									</header>

									{gallery.description ? (
										<p className="mt-3 text-sm text-slate-300">{gallery.description}</p>
									) : null}

									<div className="mt-5 flex flex-col gap-3 rounded-2xl border border-slate-800 bg-slate-900/60 p-4">
										<div className="flex flex-wrap items-center justify-between gap-3">
											<p className="text-sm text-slate-300">
												Total photos:{" "}
												<span className="font-semibold text-slate-100">{gallery.photoCount}</span>
											</p>
											<button
												type="button"
					...
					<div className="w-full max-w-md rounded-2xl border border-slate-800 bg-slate-900 p-8 text-slate-100 shadow-2xl">
						...
						<div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
							<button
								...
								className="inline-flex items-center justify-center rounded-full border border-slate-700 px-5 py-2 text-sm font-semibold uppercase tracking-wide text-slate-200 transition hover:border-slate-500 hover:text-slate-100 focus:outline-none focus:ring-2 focus:ring-main/40 disabled:cursor-not-allowed disabled:opacity-60"
							>
								Cancel
							</button>
							<button
