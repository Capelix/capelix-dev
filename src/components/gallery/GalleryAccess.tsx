...
const LOCAL_STORAGE_PREFIX = "gallery:selected_codes"
...
export default function GalleryAccess({ locale = "en" }: GalleryAccessProps) {
	...
	const selectionStorageKey = useMemo(() => {
		...
		return `${LOCAL_STORAGE_PREFIX}:${gallery.id}`
	}, [gallery?.id])
	...
	const handleVerifyPin = useCallback(
		async (event: React.FormEvent<HTMLFormElement>) => {
			...
			try {
				...
				const availableCodes = new Set(payload.photos.map((photo) => photo.image_code))
				let initialSelection = payload.selectedCodes ?? []

				if (typeof window !== "undefined") {
					try {
						const storedRaw = localStorage.getItem(`${LOCAL_STORAGE_PREFIX}:${payload.gallery.id}`)
						if (storedRaw) {
							const stored = JSON.parse(storedRaw)
							if (Array.isArray(stored)) {
								initialSelection = stored.filter((code) => availableCodes.has(code))
							}
						}
					} catch (storageError) {
						console.warn("Failed to load stored gallery selection:", storageError)
					}
				}
	...
	useEffect(() => {
		if (!selectionStorageKey || !isClient) return

		try {
			localStorage.setItem(selectionStorageKey, JSON.stringify(selectedCodes))
		} catch (storageError) {
			console.warn("Failed to persist gallery selection:", storageError)
		}
	}, [selectionStorageKey, selectedCodes, isClient])
	...
	const selectedSummary = i18n.GALLERY_SELECTED_COUNT.replace(
		"{selected}",
